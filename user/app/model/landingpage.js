...
const userlandingpagelModel = async (data, res) => {
    try {
      const hospital = await HospitalModel.countDocuments();
        ...
        const tophospital = await HospitalModel.find().limit(6).select('name image')
        const topcategories = await CategoryModel.find().limit(6).select('name description')
...
module.exports = {
    filter_user_hospital_model , userretrievesinglehospitalModel , userlandingpagelModel
}